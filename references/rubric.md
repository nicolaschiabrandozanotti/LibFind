...
- A named package does not get credit just because the team already wants it.
...
| License | Gate | Must be clear, compatible, and acceptable for expected commercial or internal use. |
| Integration cost | 10 | Setup effort, configuration, migration burden, API friction, and test impact. |
| API and documentation quality | 10 | Clear docs, examples, stable API, typed interfaces where relevant, safe usage guidance. |
| Bundle and runtime impact | 5 | Package size, transitive dependencies, runtime cost, browser or server constraints. |
...
- License creates incompatible strong-copyleft obligations for expected commercial or internal use.
...
- plugin
- framework extension
- copyable snippet or reference only
...
- Prefer `build-it-in-house` when the required behavior is small but the package introduces a broad transitive tree.
- Require stronger evidence for maintenance, security handling, compatibility, and runtime cost.
- Treat cold starts, bundle impact, and operational blast radius as first-class concerns.

### External SDK

- Compare the SDK against a small direct HTTP client if the used API surface is narrow.
- Evaluate provider lock-in, retries, credentials, rate limits, and portability before recommending it.
- Reject convenience-only SDKs that materially increase startup cost or architecture coupling.

### Dev dependency or CLI and tooling

- You may tolerate slightly lower maturity than for runtime code, but not unclear license or supply-chain risk.
- Focus pending checks on CI fit, false positives, maintenance burden, and local developer experience.

### Plugin or framework extension

- Require exact framework and runtime compatibility, not just broad ecosystem claims.
- Reject candidates that force project-wide architectural shifts for a narrow feature.

### Copyable snippet or reference only

- Do not recommend installation.
- Focus on license, provenance, correctness, and whether copying the approach is safer than adding a package.
...
| Internal implementation cost | Building correctly would be expensive. | Internal implementation is cheap, testable, and already close at hand. |
...
- `use-only-if-constraint-applies`: Passes gates only for a specific constraint or context.
...
- issue and PR handling, especially security issues
...
- transitive dependencies and runtime or bundle impact
